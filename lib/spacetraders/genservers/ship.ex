defmodule Spacetraders.Genservers.Ship do
  ...
  def start_link(ship) do
    GenServer.start_link(__MODULE__, ship, name: process_id(ship.symbol))
  ...
  @impl true
  def handle_cast(:sync, ship) do
    agent = Spacetraders.Genservers.Agent.get(ship.agent_symbol)

    attrs = case Spacetraders.Api.Agent.get_ship(agent, ship.symbol) do
      %{"data" => data} -> data
